# nf-core/darevskia: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/darevskia/usage](https://nf-co.re/darevskia/usage)
...
```console
nextflow run nf-core/darevskia --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/darevskia
```
...
First, go to the [nf-core/darevskia releases page](https://github.com/nf-core/darevskia/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
